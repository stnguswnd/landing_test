  ...
  return (
    <Card>
      ...
      {!assignment.imageUrl && !item?.passageText && (
        <p className="mt-3 text-sm text-slate-500">표시할 이미지 또는 스크립트가 없습니다.</p>
      )}
    </Card>
  );
}

export function ListeningHomework({ assignment, partMode }: { assignment: Assignment; partMode?: PartMode }) {
  ...
  return (
    <div className="grid gap-4">
        ...
        {item?.audioUrl ? (
          <>
            ...
            <p className="mt-3 rounded-lg bg-blue-50 p-3 text-sm font-semibold text-action">
              {hasListenedFullAudio ? `끝까지 들었어요. ${partMode ? "저장하기" : "완료하기"} 버튼을 누를 수 있습니다.` : `원본 MP3를 끝까지 한 번 들어주세요.`}
            </p>
          </>
        ) : (
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <ReadyStepButton
          disabled={!hasListenedFullAudio || pending}
          disabledReason={!hasListenedFullAudio ? "원본 MP3를 끝까지 한 번 들어야 완료할 수 있습니다." : undefined}
          onDisabledClick={setAlertMessage}
          onClick={() => partMode ? partMode.onSave() : setCompleteOpen(true)}
          tooltip={partMode ? partMode.tooltip ?? "음원을 끝까지 들었어요. 저장할 수 있습니다." : "음원을 끝까지 들었어요. 완료하기를 누를 수 있어요."}
        >
          {partMode ? partMode.label ?? "저장하기" : "완료하기"}
        </ReadyStepButton>
      </div>
      {!partMode && completeOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/35 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-md rounded-lg bg-white p-5 shadow-soft">
            ...
            <p className="mt-3 leading-7 text-slate-600">완료하면 선생님에게 숙제를 끝냈다고 알려줍니다.</p>
